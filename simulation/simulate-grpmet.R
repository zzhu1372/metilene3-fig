...
# shape.ratio <- c(1,1,1,1) #as.numeric(args[5])
...
asize = as.numeric(args[7])
othersize = as.integer((50-asize)/4)
groupSizes[['0']] = asize
groupSizes[['1']] = othersize
groupSizes[['2']] = othersize
groupSizes[['3']] = othersize
groupSizes[['4']] = 50 - othersize*3 - asize
...
DMR.bg <- do.call(rbind.fill, lapply(1:d.bg, function(i) {
  ...
  if (((j%%(no.type*no.dmr)) >= 4*no.dmr) && ((j%%(no.type*no.dmr)) < 5*no.dmr)) {
    ...
    if ((j%%(no.type*no.dmr)) %% 2 == 1) {
      for (gn in groupNames.L1A) {
        mean.groups[[gn]] <-
          c(
            rbeta(length(data.methyl[[gn]]), shape.1, shape.2) * (0.5+(1-0.5*(gn!='2'))*(shape.ratio[as.integer(j/(no.type*no.dmr))+1]-0.5)) + rbeta(length(data.methyl[[gn]]), shape.2, shape.1) *
              (1 - (0.5+(1-0.5*(gn!='2'))*(shape.ratio[as.integer(j/(no.type*no.dmr))+1]-0.5)))
          )
      }
      for (gn in groupNames.L1B) {
        mean.groups[[gn]] <-
    ...
    ## up-regulation
      ...
      for (gn in groupNames.L1A) {
        mean.groups[[gn]] <-
          c(
            rbeta(length(data.methyl[[gn]]), shape.2, shape.1) * (0.5+(1-0.5*(gn!='2'))*(shape.ratio[as.integer(j/(no.type*no.dmr))+1]-0.5)) + rbeta(length(data.methyl[[gn]]), shape.1, shape.2) *
              (1 - (0.5+(1-0.5*(gn!='2'))*(shape.ratio[as.integer(j/(no.type*no.dmr))+1]-0.5)))
          )
      }
      for (gn in groupNames.L1B) {
  ...
  if (((j%%(no.type*no.dmr)) >= 5*no.dmr)) {
      for (gn in groupNames.L1A) {
        # set.seed(j*as.integer(gn))
        # ccf = runif(1,0,1)
        mean.groups[[gn]] <-
          c(
            rbeta(length(data.methyl[[gn]]), shape.2, shape.1)
          )
      }
      for (gn in groupNames.L1B) {
        # set.seed(j*as.integer(gn))
...
DMR.fg <- do.call(rbind.fill, lapply(1:d.fg, function(i) {
  ...
  if (((j%%(no.type*no.dmr)) >= 5*no.dmr)) {
      for (gn in groupNames.L1A) {
        ...
        mean.groups[[gn]] <-
          c(
            rbeta(length(data.methyl[[gn]]), shape.1, shape.2)
          )
      }
      for (gn in groupNames.L1B) {
        # set.seed(j*as.integer(gn))
...
for (gn in groupNames) {
  for (i in data.methyl[[gn]]) {
    ...
    "_",
    shape.4,
    ".bed"), format(sample, scientific=F, trim=T), col.names=F, row.names=F, sep="\t", quote=F)
	j <- j+1
  }
}
